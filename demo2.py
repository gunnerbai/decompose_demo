...
def  geiFw_Info(all_contours,Fw_list,save_path,image):
     ...
     for i in range(E):
         ...
         if len(houxu_fw) != 0:
            ...
            cv2.drawContours(new_img, houxu_fw, -1, (0, 0, 0), 4)
...
def find_oth_jiaodian(d_a,d_b,d_c,d_d,bihua,al_bh):
    ...
    if len(d_b) < 3 and len(d_b) > 0:
        print('B')
        p = get_jj_p(bihua[int(d_b[0])], al_bh)
        if p == None and len(d_b) > 1:
            p = get_jj_p(bihua[int(d_b[1])], al_bh)
        jl_list.append(p)
        # jl_list['bh' + str(k)] = p
    if len(d_c) < 3 and len(d_c) > 0:
        print('C')
        p = get_jj_p(bihua[int(d_c[0])], al_bh)
        print('11111111111111111111111', p)
        if p == None and len(d_c) > 1:
            p = get_jj_p(bihua[int(d_c[1])], al_bh)
    ...
    if len(d_d) < 3 and len(d_d) > 0:
        print('D')
        p = get_jj_p(bihua[int(d_d[0])], al_bh)
        print('11111111111111111111111DDD', p)
        if p == None and len(d_d) > 1:
            p = get_jj_p(bihua[int(d_d[1])], al_bh)
        jl_list.append(p)
    return jl_list
...
img1 = cv2.imread('images/test5.png', cv2.IMREAD_GRAYSCALE)  # 读取图像
if len(img1.shape) > 2:
    h1, w1, n = img1.shape
    img2 = img1[::, 2]
else:
    h1, w1 = img1.shape
    img2 = img1
contours, cnt = cv2.findContours(img2, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)  # 提取轮廓 contours
img3 = cv2.drawContours(img2, contours, -1, (0, 0, 255, 22), 2)  # 绘制轮廓，填充
# cv2.imshow("img", img2)  # 显示原图及轮廓
# cv2.waitKey(0)  # 显示轮廓
...
print(files_name)
img = cv2.imread(files_name[-1])
img = 255-img
showImage(img)
skeleton_image ,ceshi = getSkeleton(img)
# showImage(skeleton_image,'cv2')
...
for i in ceshi:
    ...
    if len(k)>=3:
        for j in k:
            ...
            # testli11111(neighbours_x_y(i[1],i[0]),img,(255,0,0))
            # skeleton_image[neighbours_x_y(i[1], i[0])[int(j)]]==0
testli11111(tmp,img,(255,0,0))
...
# q_jz_list = getFork_posion(p_list,bihua_all_list)
...
# testli11111(list(np.uint8(np.array(weight_list))) ,img,(255,0,0))
