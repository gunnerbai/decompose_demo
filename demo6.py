...
# showImage(sk)
p_list1 = get_allWP(sk)
# 原始白色点和去掉拐角的点进行相减，得到的就是fork点
print('p_list',len(p_list))
# testli11111(p_list,img,(255,0,0))
# print('p_list1',len(p_list1))
# testli11111(p_list1,img,(255,0,0))
for i in p_list1:
    p_list.remove(i)
print(len(p_list))
print('fork',p_list)
...
# for i in all_strokes:
#     testli11111(i.points,img,(255,0,0))

# showImage(all_strokes)
bihua_all_list = []
for i in all_strokes:
    bihua_all_list.append(i.points)

# print(bihua_all_list)
# print(len(bihua_all_list))
# testli11111(p_list,img,(255,0,0))
# getFork_posion(p_list,bihua_all_list)
q_jz_list,fork_list = getFork_posion(p_list,bihua_all_list)
weight_list = get_strokes_Weight(all_strokes)
# print(q_jz_list)
# print(q_jz_list[0])
bihua_lines = []
for i in all_strokes:
    # testli11111([i.points[0]],img1,(255,0,0))
    # testli11111([i.points[-1]],img1,(255,0,0))
...
def GetCrossAngle(l1, l2):
    ...
    arr_0 = np.array([(l1[1][1] - l1[0][1]), (l1[1][0] - l1[0][1])])
    # arr_1 = np.array([(l2.p2.x - l2.p1.x), (l2.p2.y - l2.p1.y)])
    # (l2[1][0] - l2[0][0])
    # arr_1 = np.array([ (l1[1][0] - l1[0][0]),(l2[1][1] - l2[0][1]) ])
    arr_1 = np.array([ (l2[1][1] - l2[0][1]),(l2[1][0] - l2[0][0]) ])
    cos_value = (float(arr_0.dot(arr_1)) / (np.sqrt(arr_0.dot(arr_0)) * np.sqrt(arr_1.dot(arr_1))))   # 注意转成浮点数运算
    return np.arccos(cos_value) * (180/np.pi)

# keys = fork_list.keys()
# for i in keys:
#     print(fork_list.get(i))
#     testli11111(fork_list.get(i),img1,(255,0,0))


# weight_list =np.array(q_jz_list)
# for i in range(le):
#     for j in range(le):
#         if i!=j and q_jz_list[i][j]==1:
#             print('第{}条线与第{}有交集'.format(i,j))
#             # angle = GetCrossAngle(bihua_lines[i], bihua_lines[j])
#             angle = np.dot(bihua_lines[i], bihua_lines[j]) / (np.linalg.norm(bihua_lines[i]) * np.linalg.norm(bihua_lines[j]))
#             print('第{}笔画到第{}笔画的度数为{}'.format(i, j, angle))
#             testimage = cv2.imread('images/test2.png')
#             testli11111(bihua_lines[i], testimage, (255, 0, 0))
#             testli11111(bihua_lines[j], testimage, (255, 0, 0))


print('bihua_lines = ',bihua_lines)
# for i in range(len(bihua_lines)):
#     for j in range(len(bihua_lines)):
#         if i!=j:
#             angle = GetCrossAngle(bihua_lines[i], bihua_lines[j])
#             print('第{}笔画到第{}笔画的度数为{}'.format(i,j,angle))
#             # if int(angle)<50:
#             testimage = cv2.imread('images/test2.png')
#             testli11111(bihua_lines[i], testimage, (255, 0, 0))
#             testli11111([(bihua_lines[i][1][0] ,bihua_lines[i][1][1]),(bihua_lines[j][0][0] , bihua_lines[j][0][1])], testimage, (255, 0, 0))
...
# testli(tmp, skeleton_image, (0, 0, 0))
# print('111111111111',tmp)
...
#
    ...
    # print('--------------',i.points)
    # getBlo_neighbour(i.points,tmp,skeleton_image_cy)
#
...
# print(oth_p)
# print(all_strokes[0])
# before = draw_strokes(img, all_strokes, colors)
# print(len(tmp))
# for i in all_strokes:
#     print(i.points)
# print(tmp)
# before = testli(tmp,before,(111,111,111))
# displaying contours before merge
# showImage(before)


# skeleton_image ,ceshi = getSkeleton(image)
#
# showImage(skeleton_image)
# all_strokes, _  , oth_p = generate_strokes2(skeleton_image)
# print(len(all_strokes))
