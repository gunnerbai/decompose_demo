...
img1 = cv2.imread('images/test3.png', cv2.IMREAD_GRAYSCALE)  # 读取图像
if len(img1.shape) > 2:
    h1, w1, n = img1.shape
    img2 = img1[::, 2]
else:
...
img3 = cv2.drawContours(img2, contours, 0, (0, 0, 255, 22), 4)  # 绘制轮廓，填充
...
img = 255-img
showImage(img)
skeleton_image  = getSkeleton(img)
...
tmp = []
for i in ceshi:
    a = neighbours(i[1],i[0],skeleton_image)
    k = np.argwhere(a)
    if len(k)>=3:
        for j in k:
...
for i in  tmp:
    ...
    z= []
dianwei_z2 = []
for i in range(len(dianwei_z)-1):
      if i==0:
        dianwei_z2.append(dianwei_z[i])
      jl = compute_distance(dianwei_z[i][0],dianwei_z[i+1][0])
      print(jl)
      if jl>20:
          print('位置',i+1)
          dianwei_z2.append(dianwei_z[i+1])
print('dianwei_z2',len(dianwei_z2))
print('dianwei_z2',dianwei_z2)
j = 0
for i in dianwei_z2:
    print(j)
